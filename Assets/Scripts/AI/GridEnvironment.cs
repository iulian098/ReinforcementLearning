...
{
    public static GridEnvironment instance;
    ...
    public List<GridAgent> agentsList;
    ...
    public void BeginNewGame() {
        ...
        for (int i = 0; i < numberOfAgents; i++) {
            GridAgent a = Instantiate(playerPrefab, spawnPoint.position, spawnPoint.rotation);//new InternalAgent();
    ...
    public void SetUp()
    {
        agentsList = new List<GridAgent>(numberOfAgents);
        gridGen.Init();
    ...
    public void Run(GridAgent agent, bool isLast = false) {
    ...
    public void Step(GridAgent agent, bool isLast) {
        if (isLast)
    ...
    public void SetEValue() {
        foreach (GridAgent agent in agentsList)
            agent.SetEValue(0.1f);
    }

    public IEnumerator WaitStep() {
        yield return new WaitForSeconds(waitTime);
        foreach (GridAgent a in agentsList) {
            EndStep(a);
    ...
    public void MiddleStep(int action, GridAgent agent)
    {
        /*
        0 - Acc, 
        1 - Brake, 
    ...
    public void EndStep(GridAgent agent) {
    ...
    public void EndReset(GridAgent agent) {
        agent.SendState(new Vector2Int((int)agent.transform.position.x, (int)agent.transform.position.z), done);
        skippingFrames = false;
